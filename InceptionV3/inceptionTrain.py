...
logger = logging.getLogger("PyTorch_InceptionV3_Classifier")

# Config parameters (hyperparameters)
CONFIG = {
    "csv_path": "visionTransformer/new_synthetic_dataset.csv",  # Path to your CSV file
    "img_height": 299,  # InceptionV3 default input height
    "img_width": 299,   # InceptionV3 default input width
    ...
    "wandb_project": "inceptionv3_classifier_",  # WandB project name
    "wandb_entity": "nevin-as-no",  # Your WandB username (None will use default)
    "wandb_tags": ["inceptionv3", "pytorch", "image-classification"],
...
def setup_wandb():
    ...
    wandb.run.name = f"inceptionv3_bs{CONFIG['batch_size']}_e{CONFIG['epochs']}_lr{CONFIG['learning_rate']}"
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = self.dataframe.iloc[idx]['caption']
...
def build_model():
    """Build an InceptionV3 model fine-tuned for the classification task."""
    ...
    # Get the number of features in the AuxLogits classifier
    num_ftrs_aux = model.AuxLogits.fc.in_features
    # Replace the AuxLogits classifier
    model.AuxLogits.fc = nn.Sequential(
        nn.Dropout(p=CONFIG["dropout_rate"]),
        nn.Linear(num_ftrs_aux, CONFIG["num_classes"])
    )
    
    # Get the number of features in the final classifier
...
def train_epoch(model, train_loader, criterion, optimizer, epoch):
    ...
    for inputs, labels in pbar:
        ...
        # Forward pass
        # InceptionV3 returns a tuple when training (main output, aux output)
...
def validate(model, val_loader, criterion, epoch, phase="val"):
    ...
    with torch.no_grad():
        for inputs, labels in pbar:
            ...
            # For inception during validation/testing, it returns only the main output
            if isinstance(outputs, tuple):
                outputs = outputs[0]
...
def fine_tune_model(model, train_loader, val_loader, criterion):
    ...
    # For InceptionV3, we'll unfreeze the last 2 blocks
    ct = 0
    for child in model.children():
        ct += 1
        if ct >= 17:  # Unfreeze approximately the last 2 inception blocks
...
def main():
    ...
    try:
        logger.info("Starting PyTorch InceptionV3 classification training pipeline")
