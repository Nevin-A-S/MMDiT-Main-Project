...
logger = logging.getLogger("PyTorch_EfficientNet_Classifier")

# Config parameters (hyperparameters)
CONFIG = {
    "csv_path": "visionTransformer/new_synthetic_dataset.csv",  # Path to your CSV file
    "img_height": 224,  # EfficientNet default input height
    "img_width": 224,   # EfficientNet default input width
    ...
    "wandb_project": "efficientnet_classifier_pytorch",  # WandB project name
    "wandb_entity": None,  # Your WandB username (None will use default)
    "wandb_tags": ["efficientnet", "pytorch", "image-classification"],
...
def setup_wandb():
    ...
    wandb.run.name = f"efficientnet_{CONFIG['efficientnet_version']}_bs{CONFIG['batch_size']}_e{CONFIG['epochs']}_lr{CONFIG['learning_rate']}"
...
def build_model():
    """Build an EfficientNet model fine-tuned for the classification task."""
    logger.info(f"Building EfficientNet-{CONFIG['efficientnet_version']} model")
    
    # Load pre-trained EfficientNet model
    if CONFIG['efficientnet_version'] == 'b0':
        model = models.efficientnet_b0(pretrained=True)
    elif CONFIG['efficientnet_version'] == 'b1':
        model = models.efficientnet_b1(pretrained=True)
    elif CONFIG['efficientnet_version'] == 'b2':
        model = models.efficientnet_b2(pretrained=True)
    elif CONFIG['efficientnet_version'] == 'b3':
        model = models.efficientnet_b3(pretrained=True)
    elif CONFIG['efficientnet_version'] == 'b4':
        model = models.efficientnet_b4(pretrained=True)
    elif CONFIG['efficientnet_version'] == 'b5':
        model = models.efficientnet_b5(pretrained=True)
    elif CONFIG['efficientnet_version'] == 'b6':
        model = models.efficientnet_b6(pretrained=True)
    elif CONFIG['efficientnet_version'] == 'b7':
        model = models.efficientnet_b7(pretrained=True)
    else:
        logger.warning(f"Invalid EfficientNet version specified. Defaulting to b0.")
        model = models.efficientnet_b0(pretrained=True)
...
def fine_tune_model(model, train_loader, val_loader, criterion):
    ...
    # For EfficientNet, we'll unfreeze the last blocks (features.7 and features.8)
    for name, child in model.named_children():
        if name == 'features':
            # Unfreeze the last two blocks (adjust as needed for different EfficientNet versions)
            for i, block in enumerate(child):
                if i >= len(child) - 2:  # Last two blocks
...
def main():
    ...
    try:
        logger.info("Starting PyTorch EfficientNet classification training pipeline")
