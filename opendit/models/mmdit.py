...
class MMDiT(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                MMDitBlocks(dim_cond = hidden_size,
                dim_text = hidden_size,
                dim_image = hidden_size,
                qk_rmsnorm = True,
                heads = num_heads,
                flash_attn = True)
        ...
        self.final_layer = MMDiTFinalLayer(
            hidden_size, np.prod(patch_size) if use_video else patch_size , self.out_channels
        )
    ...
    def forward(self, x, t, c):
        ...
        for block in self.blocks:
            ...
            # else:
            x , c = block(time_cond = t, image_tokens = x, text_tokens = c)  # (N, T, D)
