...
class MMDiTBlock(nn.Module):
    ...
    def forward(self, y, x, c):
        ...
        # c = bc(alpha_txt).view(2, 1, 1).expand(-1, 154, 384) * c + beta_txt * torch.ones_like(alpha_txt)

        bc_alpha = bc(alpha_txt).view(2, 1, 1)  # Squeeze extra dimensions
        bc_alpha = bc_alpha.expand(-1, 154, 384)  # Expand to match `c` shape

        beta_txt_expanded = beta_txt.view(2, 1, 1).expand(-1, 154, 384)  # Match `c` shape

        ones_like_c = torch.ones_like(c)  # Shape [2, 154, 384]

        c = bc_alpha * c + beta_txt_expanded * ones_like_c

        # x = bc(alpha_pix) * x + beta_pix * torch.ones_like(alpha_pix)


        # Adjust bc(alpha_pix)
        bc_alpha = bc(alpha_pix).view(2, 1, 1)  # Remove extra dimensions
        bc_alpha = bc_alpha.expand(-1, 16, 384)  # Match shape of x

        # Adjust beta_pix
        beta_pix_expanded = beta_pix.view(2, 1, 1).expand(-1, 16, 384)  # Match shape of x

        # Perform the operation
        x = bc_alpha * x + beta_pix_expanded
...
class MMDiTFinalLayer(nn.Module):
    ...
    def forward(self, x, c):
        ...
        x = modulate(nn.Identity(), x, shift, scale)
