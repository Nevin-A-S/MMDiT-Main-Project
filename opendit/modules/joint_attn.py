...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        print("org shape",orig_shape)
        
        # Use reshape instead of view for better handling of non-contiguous tensors
        x_reshaped = x.reshape(-1, x.shape[-1])
        
        if self.p < 0. or self.p > 1.:
            norm_x = x_reshaped.norm(2, dim=-1, keepdim=True)
            d_x = x_reshaped.shape[-1]
        else:
            partial_size = int(x_reshaped.shape[-1] * self.p)
            partial_x, _ = torch.split(x_reshaped, [partial_size, x_reshaped.shape[-1] - partial_size], dim=-1)
            norm_x = partial_x.norm(2, dim=-1, keepdim=True)
            d_x = partial_size

        rms_x = norm_x * d_x ** (-1. / 2)
        x_normed = x_reshaped / (rms_x + self.eps)

        # Ensure scale parameter matches the feature dimension
        if self.scale.shape[0] != x_reshaped.shape[-1]:
            raise ValueError(f"Scale parameter dimension ({self.scale.shape[0]}) does not match input feature dimension ({x_reshaped.shape[-1]})")

        if self.bias:
            output = self.scale * x_normed + self.offset
        else:
            output = self.scale * x_normed
...
class JointAttention(nn.Module):
    def __init__(self, txt_dim, pix_dim,
        ...
        assert txt_dim % num_heads == 0, 'txt_dim should be divisible by num_heads'
        assert pix_dim % num_heads == 0, 'pix_dim should be divisible by num_heads'
        self.num_heads = num_heads
        self.txt_head_dim = txt_dim // num_heads
        self.pix_head_dim = pix_dim // num_heads
        self.txt_scale = self.txt_head_dim ** -0.5
        self.pix_scale = self.pix_head_dim ** -0.5
        self.attention_mode = attention_mode
        
        # Separate QKV projections for text and pixel streams
        self.qkv_text = nn.Linear(txt_dim, txt_dim * 3, bias=qkv_bias)
        self.qkv_pix = nn.Linear(pix_dim, pix_dim * 3, bias=qkv_bias)
        ...
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj_pix = nn.Linear(pix_dim, pix_dim)
        self.proj_text = nn.Linear(txt_dim, txt_dim)
        self.proj_drop_text = nn.Dropout(proj_drop)
        self.proj_drop_pix = nn.Dropout(proj_drop)
    ...
    def split_qkv(self, qkv, B, N, total_N, num_heads, dim):
        """
        Split QKV tensor into query, key, and value tensors
        Args:
            qkv: Input tensor
            B: Batch size
            N: Sequence length
            total_N: Total sequence length (N * sequence_parallel_size)
        ...
        if self.sequence_parallel_type == "ulysses":
            ...
            if self.enable_flashattn:
                q = q.reshape(B, N * self.sequence_parallel_size, num_heads, dim)
                k = k.reshape(B, N * self.sequence_parallel_size, num_heads, dim)
                v = v.reshape(B, N * self.sequence_parallel_size, num_heads, dim)
            else:
                q = q.reshape(B, N * self.sequence_parallel_size, num_heads, dim).permute(0, 2, 1, 3)
                k = k.reshape(B, N * self.sequence_parallel_size, num_heads, dim).permute(0, 2, 1, 3)
                v = v.reshape(B, N * self.sequence_parallel_size, num_heads, dim).permute(0, 2, 1, 3)
        else:
            # Split into q, k, v and reshape
            qkv = qkv.reshape(B, N, 3, num_heads, dim)
            if self.enable_flashattn:
    ...
    def forward(self, x, c):
        ...
        qkv_pix = self.qkv_pix(x)
        num_heads = self.num_heads if self.sequence_parallel_type is None else self.num_heads // self.sequence_parallel_size
        q_pix, k_pix, v_pix = self.split_qkv(qkv_pix, B, N, total_N, num_heads, self.pix_head_dim)

        print("qkv shape",qkv_pix.shape)
        print("q shape",q_pix.shape)
        print("k shape",k_pix.shape)
        print("v shape",v_pix.shape)

        q_pix = self.rms_q_pix(q_pix)
        k_pix = self.rms_k_pix(k_pix)

        # Process text stream
        qkv_text = self.qkv_text(c)
        q_text, k_text, v_text = self.split_qkv(qkv_text, B, N, total_N, num_heads, self.txt_head_dim)
        
        if self.attention_mode != 'rebased':
            q_text = self.rms_q_text(q_text)
            k_text = self.rms_k_text(k_text)

        # Handle attention based on mode
        if self.attention_mode == 'xformers':
            z_pix = xformers.ops.memory_efficient_attention(q_pix, k_pix, v_pix)
            z_text = xformers.ops.memory_efficient_attention(q_text, k_text, v_text)
        
        elif self.attention_mode == 'flash':
            with torch.backends.cuda.sdp_kernel(enable_math=False, enable_flash=True, enable_mem_efficient=False):
                z_pix = torch.nn.functional.scaled_dot_product_attention(
                    q_pix, k_pix, v_pix, dropout_p=self.attn_drop.p, scale=self.pix_scale
                )
                z_text = torch.nn.functional.scaled_dot_product_attention(
                    q_text, k_text, v_text, dropout_p=self.attn_drop.p, scale=self.txt_scale
                )
        
        elif self.attention_mode == 'math':
            attn_pix = (q_pix @ k_pix.transpose(-2, -1)) * self.pix_scale
            attn_pix = attn_pix.softmax(dim=-1)
            attn_pix = self.attn_drop(attn_pix)
            z_pix = attn_pix @ v_pix

            attn_text = (q_text @ k_text.transpose(-2, -1)) * self.txt_scale
            attn_text = attn_text.softmax(dim=-1)
            attn_text = self.attn_drop(attn_text)
            z_text = attn_text @ v_text
        
        elif self.attention_mode == 'rebased':
            z_pix = parallel_rebased(q_pix, k_pix, v_pix, self.eps, True, True)
            z_text = parallel_rebased(q_text, k_text, v_text, self.eps, True, True)
        
        elif self.attention_mode == 'ring':
            z_pix = ring_flash_attn_cuda(q_pix, k_pix, v_pix, causal=self.causal, bucket_size=self.ring_bucket_size)
            z_text = ring_flash_attn_cuda(q_text, k_text, v_text, causal=self.causal, bucket_size=self.ring_bucket_size)
        
        else:
            raise NotImplementedError(f"Attention mode {self.attention_mode} not implemented")

        # Reshape output
        if self.sequence_parallel_type is None:
            z_pix_shape = (B, N, self.pix_dim)
            z_text_shape = (B, N, self.txt_dim)
        else:
            z_pix_shape = (B, total_N, num_heads * self.pix_head_dim)
            z_text_shape = (B, total_N, num_heads * self.txt_head_dim)

        if self.attention_mode in ['flash', 'ring', 'rebased']:
            z_pix = z_pix.reshape(z_pix_shape)
            z_text = z_text.reshape(z_text_shape)
        else:
            z_pix = z_pix.transpose(1, 2).reshape(z_pix_shape)
            z_text = z_text.transpose(1, 2).reshape(z_text_shape)

        if self.sequence_parallel_size > 1:
            z_pix = all_to_all_comm(z_pix, self.sequence_parallel_group, scatter_dim=1, gather_dim=2)
            z_text = all_to_all_comm(z_text, self.sequence_parallel_group, scatter_dim=1, gather_dim=2)

        x = self.proj_pix(z_pix)
        x = self.proj_drop_pix(x)

        c = self.proj_text(z_text)
        c = self.proj_drop_text(c)
