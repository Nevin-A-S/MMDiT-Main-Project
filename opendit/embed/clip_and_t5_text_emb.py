...
class TextEmbeddingProjector(nn.Module): # Using instead of Label Embedder
    def __init__(self, clip_in_text_channels, t5_in_text_channels, text_hidden_size, timestep_hidden_size, bias=True):
        ...
        print("clip_in_text_channels",clip_in_text_channels)
        print("t5_in_text_channels",t5_in_text_channels)

        self.proj = nn.Linear(clip_in_text_channels, text_hidden_size, bias=bias)
    ...
    def forward(self,
        ...
        if C_T5 > C:
            clip_embeds = nn.functional.pad(clip_embeds, (0, C_T5 - C), mode='constant', value=0)
...
class MixedTextEmbedder(nn.Module):
    ...
    def forward(self, text_prompts, train, force_drop_ids=None):
        ...
        text_prompts = [str(text_prompt) for text_prompt in text_prompts]
